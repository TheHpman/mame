...
public:
	...
	std::unique_ptr<bitmap_ind16> m_mixer_output_bitmap;
	std::unique_ptr<bitmap_ind16> m_mixer_attr_bitmap;
	...
	void videodac_init(bitmap_ind16 **bitmaps);
	void videodac_update(bitmap_ind16 **bitmaps, const rectangle &cliprect);
};

void piratesh_state::videodac_init(bitmap_ind16 **bitmaps)
{
	m_mixer_output_bitmap = std::make_unique<bitmap_ind16>(bitmaps[0]->width(), bitmaps[0]->height());
	m_mixer_attr_bitmap = std::make_unique<bitmap_ind16>(bitmaps[0]->width(), bitmaps[0]->height());
}

void piratesh_state::videodac_update(bitmap_ind16 **bitmaps, const rectangle &cliprect)
{
	bitmap_ind16 *mbitmaps[4] = {
		bitmaps[kvideodac_device::BITMAP_COLOR],
		bitmaps[kvideodac_device::BITMAP_ATTRIBUTES],
		m_mixer_output_bitmap.get(),
		m_mixer_attr_bitmap.get()
	};

	m_mixer->bitmap_update(mbitmaps, cliprect);
}

void piratesh_state::mixer_init(bitmap_ind16 **bitmaps)
{
	bitmaps[k055555_device::LAYERA_COLOR]->fill(0);
	bitmaps[k055555_device::LAYERB_COLOR]->fill(0);
	bitmaps[k055555_device::LAYERC_COLOR]->fill(0);
	bitmaps[k055555_device::LAYERD_COLOR]->fill(0);
...
void piratesh_state::mixer_update(bitmap_ind16 **bitmaps, const rectangle &cliprect)
{
	...
	m_lvc->bitmap_update(bitmaps[k055555_device::LAYERS2_COLOR],
						 bitmaps[k055555_device::LAYERS2_ATTR],
						 cliprect);
	m_sprites->bitmap_update(bitmaps[k055555_device::LAYERS3_COLOR],
							 bitmaps[k055555_device::LAYERS3_ATTR],
							 cliprect);
}
...
static MACHINE_CONFIG_START( piratesh )
	...
	MCFG_KVIDEODAC_SET_INIT_CB(DEVICE_SELF, piratesh_state, videodac_init)
	MCFG_KVIDEODAC_SET_UPDATE_CB(DEVICE_SELF, piratesh_state, videodac_update)
	...
	MCFG_K055555_SET_INIT_CB(DEVICE_SELF, piratesh_state, mixer_init)
	MCFG_K055555_SET_UPDATE_CB(DEVICE_SELF, piratesh_state, mixer_update)
