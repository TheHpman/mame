...
void k053251_device::bitmap_update(bitmap_ind16 **bitmaps, const rectangle &cliprect)
{
	...
	for(int y = cliprect.min_y; y <= cliprect.max_y; y++) {
		const u16 *c0 = &m_bitmaps[LAYER0_COLOR]->pix16(y, cliprect.min_x);
		const u16 *c1 = &m_bitmaps[LAYER1_COLOR]->pix16(y, cliprect.min_x);
		const u16 *c2 = &m_bitmaps[LAYER2_COLOR]->pix16(y, cliprect.min_x);
		const u16 *c3 = &m_bitmaps[LAYER3_COLOR]->pix16(y, cliprect.min_x);
		const u16 *c4 = &m_bitmaps[LAYER4_COLOR]->pix16(y, cliprect.min_x);
		const u16 *a0 = &m_bitmaps[LAYER0_ATTR ]->pix16(y, cliprect.min_x);
		const u16 *a1 = &m_bitmaps[LAYER1_ATTR ]->pix16(y, cliprect.min_x);
		const u16 *a2 = &m_bitmaps[LAYER2_ATTR ]->pix16(y, cliprect.min_x);
		u16 *dc = &bitmaps[kvideodac_device::BITMAP_COLOR]->pix16(y, cliprect.min_x);
		u16 *da = &bitmaps[kvideodac_device::BITMAP_ATTRIBUTES]->pix16(y, cliprect.min_x);

		const u16 *const *shadp = m_shadow_layer == LAYER0_ATTR ? &a0 : m_shadow_layer == LAYER1_ATTR ? &a1 : &a2;
