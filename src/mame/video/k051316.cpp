...
void k051316_device::set_info(int tile_bpp, int color_base_line, int color_line_count, bool ram_based)
{
	m_tile_bpp = tile_bpp;
	m_color_base_line = color_base_line;
	m_color_line_count = color_line_count;
...
{
	u32 cx = cliprect.min_x - 21;
	u32 cy = cliprect.min_y - 16;
	...
	u32 pal_mask = ((1 << m_color_line_count) - 1) << m_color_base_line;
	u32 pal_shift = m_color_base_line - m_tile_bpp;
	...
	for(int y = cliprect.min_y; y <= cliprect.max_y; y++) {
		...
		for(int x = cliprect.min_x; x <= cliprect.max_x; x++) {
			...
			if(!outside) {
				...
				if(adr & 0x400000)
					adr ^= 0x00000f;
				// possibly y on adr & 0x800000, unknown
				*pc++ = ((adr & pal_mask) >> pal_shift) | rom_lookup(adr);
