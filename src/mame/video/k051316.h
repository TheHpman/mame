...
#define MCFG_K051316_ADD(_tag, _tile_bpp, _color_base_line, _color_line_count, _ram_based) \
	MCFG_DEVICE_ADD(_tag, K051316, 0) \
		downcast<k051316_device *>(device)->set_info(_tile_bpp, _color_base_line, _color_line_count, _ram_based);

#define MCFG_K051316_WRAP(_wrap) \
	downcast<k051316_device *>(device)->set_wrap(_wrap);


class k051316_device : public device_t, public flow_render::interface
{
public:
	...
	*/
	...
	void set_info(int tile_bpp, int color_base_line, int color_line_count, bool ram_based);
...
private:
	...
	int m_tile_bpp, m_color_base_line, m_color_line_count;
