...
void kvideodac_device::bitmap_update(bitmap_rgb32 *bitmap, const rectangle &cliprect)
{
	...
	for(int y = cliprect.min_y; y <= cliprect.max_y; y++) {
		const u16 *bcolor = &m_bitmaps[BITMAP_COLOR]->pix16(y, cliprect.min_x);
		const u16 *battr  = &m_bitmaps[BITMAP_ATTRIBUTES]->pix16(y, cliprect.min_x);
		u32 *dest = &bitmap->pix32(y, cliprect.min_x);
...
u32 kvideodac_device::screen_update(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
